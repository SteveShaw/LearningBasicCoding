...
从\fcj{root}开始，先将\fcj{root}压入栈，然后再将其所有left child nodes压入栈，然后取出栈顶node，保存节点值，再将当前指针移到其right child node上，若存在right child node，则在下次循环时又可将其所有left child nodes压入栈中。这样就保证了访问顺序为left--root--right。
